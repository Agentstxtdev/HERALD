 ...
 * replaced so the agentify-managed values win: we own those paths, the user
...
export function headersDeploymentNote(platform: HostingPlatform): string {
  switch (platform) {
    ...
    case 'vercel':
      return 'Vercel: `vercel.json#headers` is applied at the edge. Existing entries are preserved; only the agentify-managed paths (§4.5 plus declared A2A AgentCards) were touched.'
    case 'unknown':
      return (
