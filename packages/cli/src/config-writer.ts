...
// ─────────────────────────────────────────────────────────────────────────────
// Config writer — pure template generation + file writing for agentic.config.js.
...
// See: https://github.com/agentstxt/agents.txt/tree/main/examples/nextjs`,
  express: `// Express — app.use(createAgenticRouter(config)) + app.use('/api', agenticPaymentMiddleware(config))
// See: https://github.com/agentstxt/agents.txt/tree/main/examples/express`,
  hono: `// Hono — createAgenticRoutes(app, config) + app.use('/api/*', agenticPaymentMiddleware(config))`,
  astro: `// Astro / static — run: npx agentify generate --out ./public`,
  unknown: `// Static sites — run: npx agentify generate --out ./public
// Server — import { createAgenticRouter, agenticPaymentMiddleware } from '@agentify/web/express'`,
}
...
export function buildAgenticConfigContent(choices: AgenticConfigChoices): string {
  ...
  return `// agentic.config.js — generated by agentify init
// Docs: https://github.com/agentstxt/agents.txt
